...
-(void)connectPeripheral:(CBPeripheral*)peripheral
{
    if (peripheral.state != CBPeripheralStateConnected){
        // 连接设备
        NSLog(@"开始连接:peripheral=%@",peripheral);
...
//        NSLog(@"扫描到设备 RSSI=%f",dist);
    ...
    if ([_activeCentralManager isEqual:central]) {
        ...
        if(![_peripherals containsObject:peripheral]){
            //根据名字过滤
            if([peripheral.name containsString:@"Smart Cup"] || [peripheral.name containsString:@"smart cup"]
               || [peripheral.name containsString:@"Smart cup"] || [peripheral.name containsString:@"smart Cup"]){
                [_peripherals addObject:peripheral];
                NSLog(@"didDiscoverPeripheral _peripherals=%@ RSSI=%f",_peripherals, dist);
                ...
                if ([[AutoConnectManager sharedManager] isLastConnected:peripheral]) {
                    NSLog(@"自动连接");
...
-(void)centralManager:(CBCentralManager *)central didConnectPeripheral:(CBPeripheral *)peripheral{
    ...
    if ([_activeCentralManager isEqual:central]) {
        ...
        //todo:设置delegate，开始扫描peripheral的service
        ...
        [[AutoConnectManager sharedManager] updateLastConnectedDevice:peripheral];
