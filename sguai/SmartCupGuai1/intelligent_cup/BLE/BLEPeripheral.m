    ...
    if (!error) {
        if ([peripheral isEqual:_activePeripheral]){
            ...
            for (CBCharacteristic *characteristic in service.characteristics){
                //6E400002-B5A3-F393-E0A9-E50E24DCCA00 write
                //6E400003-B5A3-F393-E0A9-E50E24DCCA00 read
                NSLog(@"Discovered read characteristics:%@ for service: %@", characteristic.UUID, service.UUID);
                if ([[characteristic UUID] isEqual:[CBUUID UUIDWithString:@"6E400002-B5A3-F393-E0A9-E50E24DCCA00"]]){
                    _transDataCharateristic = characteristic;
                    enWrite = YES;
                    if(enNotify){
                        [self finishConnected];
                    }
                    NSLog(@"更新write characteristics");
                }else  if ([[characteristic UUID] isEqual:[CBUUID UUIDWithString:@"6E400003-B5A3-F393-E0A9-E50E24DCCA00"]]){
                    _readCharateristic = characteristic;
                    [peripheral setNotifyValue:YES forCharacteristic:_readCharateristic];
                    enNotify = YES;
                    if(enWrite){
                        [self finishConnected];
                    }
                    NSLog(@"更新read characteristics");
                }
            }
            return;
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error{
    if ([error code] == 0) {
        ...
        if ([peripheral isEqual:_activePeripheral]){
            if ([[characteristic UUID] isEqual:[CBUUID UUIDWithString:@"6E400003-B5A3-F393-E0A9-E50E24DCCA00"]]){
...
-(void)readData{
    [self readValue:_activePeripheral characteristicUUID:_readCharateristic];
}
